...
def finchev_b(n,red):
	schub=chevalley_b(n,red[0],[red[1]])
	j=2
	if len(red)==2:
		schub1=schub
	else:
...
def pchev_b(n,k,red):
	not_p_schub=finchev_b(n,red)
	p_schub=[]
	for i in range(len(not_p_schub)):
		for j in range(len(mincp_b(n,k,len(red)))):
			if not_p_schub[i][1]==mincp_b(n,k,len(red))[j]:
				p_schub.append(not_p_schub[i])
	return p_schub
		
def pchev1_b(n,k,red):
	not_p_schub=finchev_b(n,red)	
	p_schub=[]
	for i in range(len(not_p_schub)):
		if not_p_schub[i][1] in mincp_b(n,k,len(red)):
			p_schub.append(not_p_schub[i])
	return p_schub