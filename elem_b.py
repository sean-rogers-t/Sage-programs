...
def chev_tot_b(n,k,h):
	...
	lst=[(Gen[-i-1]-Gen[-i-2])^2 for i in range(1,k)] #gives image under associated Springer map
	lst.append((2*Gen[-1]-Gen[-2])^2)
...
def boom_b(n,k,h):
	chevy=chev_tot_b(n,k,h)
	deco=[chevy[i][1] for i in range(len(chevy))]
	cof=[chevy[i][0] for i in range(len(chevy))]

	multy_chev=[multlist(cof[i],finchev_b(n,deco[i])) for i in range(len(chevy))]
	one_list=[]
